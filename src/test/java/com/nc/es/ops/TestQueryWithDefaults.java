...
public class TestQueryWithDefaults {
	...
	public void run() {
		...
		// By Default, UNMAPPED_ATTR is analyzed, so it's lower cased
		mrTwo = bound.query(ProjectionTwo.class, RootQuery.wildcard("UNMAPPED_ATTR", "DYN*"), "NM_AIRLINE_DESIGNATOR", "UNMAPPED_ATTR");
		vals = mrTwo.vals().map(v -> v.attr).filter(v -> v != null).collect(Collectors.toSet());
		Assert.assertEquals(0, vals.size());

		// Keyword is not analyzed so we can query as it is
		mrTwo = bound.query(ProjectionTwo.class, RootQuery.wildcard("UNMAPPED_ATTR.keyword", "DYN*"), "NM_AIRLINE_DESIGNATOR", "UNMAPPED_ATTR");
		vals = mrTwo.vals().map(v -> v.attr).filter(v -> v != null).collect(Collectors.toSet());
		Assert.assertEquals(4, vals.size());
		Assert.assertTrue(vals.contains("DYNAMIC1"));
		Assert.assertTrue(vals.contains("DYNAMIC2"));
		...
		mrTwo = bound.query(ProjectionTwo.class, RootQuery.wildcard("UNMAPPED_ATTR", "DYN*".toLowerCase()), "NM_AIRLINE_DESIGNATOR", "UNMAPPED_ATTR");
		vals = mrTwo.vals().map(v -> v.attr).filter(v -> v != null).collect(Collectors.toSet());
		Assert.assertEquals(4, vals.size());
		Assert.assertTrue(vals.contains("DYNAMIC1"));
		Assert.assertTrue(vals.contains("DYNAMIC2"));
	...
	public void setup() {
		...
		final IndexDefinition def = IndexDefinition.get().with(Mappings.get().withDefaults(IndexedType.get().disableAll().with("only_keywords", DynamicTemplate.of(GeneralType.STRING).matching("NM*").mappingTo(Property.get().with(Type.KEYWORD)))));
