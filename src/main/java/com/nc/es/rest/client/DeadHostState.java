...
/**
 * Holds the state of a dead connection to a host. Keeps track of how many failed attempts were
 * performed and when the host should be retried (based on number of previous failed attempts).
...
final class DeadHostState {
	...
	/**
	 * We keep track of how many times a certain node fails consecutively. The higher that number is
	 * the longer we will wait to retry that same node again. Minimum is 1 minute (for a node the
	 * only failed once), maximum is 30 minutes (for a node that failed many consecutive times).
	 */
	...
	public String toString() {
		return "DeadHostState{" + "failedAttempts=" + failedAttempts + ", deadUntilNanos=" + deadUntilNanos + '}';
	}
}